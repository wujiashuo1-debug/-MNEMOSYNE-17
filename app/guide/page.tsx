...
const surfaceEvidence = [
  ...
  ["LOG-05", "六分十四秒漂移", "门禁校时 → 执行漂移校正 → 点击“钉入矛盾记录”。"],
  ["AUD-06", "同时出现的女声", "声场回放拖到 00:14—00:19，分别监听“观察室”和“服务器室”，答案选“同一声纹同时出现”。"],
  ["IMG-07", "被擦除的第三人", "图像对照拖动遮罩，答案选择“沈雁”，点击“记录差分”。"],
  ...
  ["CCTV-11", "七次相同的巡廊", "完成“每日巡廊”全部七段核验，答案表见下方。"],
] as const;

const corridorAnswers = [
  ["第 1 天", "两只钟都停在 04:17", "建立所有片段共用的时间基线。"],
  ["第 2 天", "档案车", "轮声移动了，但画面中的档案车没有移动。"],
  ["第 3 天", "尽头凸面镜", "走廊检测为 0，镜中却被人脸检测到 1 人。"],
  ["第 4 天", "106 后出现房间", "图纸将 106 标为实心墙。"],
  ["第 5 天", "玩家观看页面时", "系统先检测到“被观看”，才渲染镜中人物。"],
  ["第 6 天", "门禁记录显示所有房间为空", "门后声纹与空房记录冲突。"],
  ["第 7 天", "106 内部递归出现同一走廊", "普通摄像机无法拍到无限递归的自身画面。"],
] as const;
  ...
  return (
    <main className="manual-shell" style={{ "--archive-image": `url("${basePath}/archive-b2.png")` } as CSSProperties}>
      ...
      <section id="quick-route" className="quick-route">
        ...
        <ol>
          <li><b>选角色：</b>拆封共同背景 3 份记录，再打开所选角色的 3 段记忆。</li>
          <li><b>拿表层证据：</b>优先取得 WEB-02、DB-03、DB-04、LOG-05、AUD-06、IMG-07、DOM-09。</li>
          <li><b>跑完巡廊：</b>完成七天问题，取得 CCTV-11。</li>
      ...
      <section className="manual-content">
        <ManualSection id="start" number="01" title="开局、角色与私人任务" summary="角色不改变谜题答案，但会改变叙事视角、私人任务和结局余波。">
          <div className="manual-copy">
            <h3>进入游戏</h3>
            <p>点击“校验并挂载数字遗物”，拆封三份共同背景记录。选择“调查员模式”不会降低谜题内容，只会显示推荐工具和渐进提示。</p>
        ...
        <ManualSection id="corridor" number="03" title="七日巡廊答案表" summary="七段可以任意顺序完成；全部正确后自动取得 CCTV-11。">
